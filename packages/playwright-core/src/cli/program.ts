...
import {
  openTraceInBrowser,
  openTraceViewerApp,
} from "../server/trace/viewer/traceViewer";
...
export const installBrowser = async function (
  ...
  onProgress:
    | ((percent: number, filename: string) => void)
    | undefined = undefined
): Promise<string> {
...
program
  ...
  .action(
    (
      ...
      options: { withDeps?: boolean; force?: boolean; dryRun?: boolean },
      onProgress:
        | ((perc: number, filename: string) => void)
        | undefined = undefined
    ) => {
      installBrowser(args, options, onProgress);
    }
  )
  .addHelpText(
    "afterAll",
...
program
  ...
  .action(function (traces, options) {
    ...
    if (options.port !== undefined || options.host !== undefined) {
      ...
      openTraceViewerApp(traces, options.browser, openOptions)
        .then((page) => {
  ...
  context.on("page", async (initial_page: Page) => {
    ...
    initial_page.on("load", async (page: Page) => {
      if (rejectUrl !== undefined && rejectUrl(page.url())) {
        ...
        await page.evaluate(() =>
          alert("You weren't allowed to access that website.")
        );
      }
...
export async function codegen(
  ...
  const { context, launchOptions, contextOptions, closeBrowser } =
    await launchContext(
      options,
      !!process.env.PWTEST_CLI_HEADLESS,
      process.env.PWTEST_CLI_EXECUTABLE_PATH,
...
function buildBasePlaywrightCLICommand(
  cliTargetLang: string | undefined
): string {
